                        ...
                        <div class="row mt-1">
                            ...
                            <div class="col-lg-6 pb-2">
                                ...
                                >
                                    ...
                                    <small
                                        class="text-danger"
                                        v-if="errors.issuer_id"
                                        v-text="errors.issuer_id[0]"
                                    ></small>
                                </div>
                            </div>
                            ...
                            <div class="col-4 form-group">
                                <label class="control-label"
                                    ...
                                    <a
                                        href="#"
                                        @click.prevent="openDialogPersonForm(3)"
                                        >[+ Nuevo]</a
                                    >
                                </label>
                                <el-select
                            ...
                            <!-- Pagos -->
                            <div class="col-12 pt-3" v-if="form.payments">
                                <table>
                                    <thead>
                                        <tr width="100%">
                                            <th>F. Pago</th>
...
<script>
import DriverForm from "../dispatches/drivers/form.vue";
...
export default {
    ...
    async created() {
        ...
        await this.$http.get(`/${this.resource}/tables`).then((response) => {
            ...
            //si customers es un objeto coge sus valores y convierte en array
            if (typeof customers === "object") {
                customers = Object.values(customers);
            }
            this.all_customers = customers.filter((c) => !c.is_driver);
            this.drivers = customers.filter((c) => c.is_driver);
        ...
        this.$eventHub.$on("reloadDataPersons", async (sender_id) => {
            await this.reloadDataCustomers(sender_id);
        });
    ...
    methods: {
        searchRemoteCustomersDrivers(input) {
            if (input.length > 0) {
                this.loading_search = true;
                let parameters = `input=${input}&driver=true`;

                this.$http
                    .get(`/sale-notes/search/customers?${parameters}`)
                    .then((response) => {
                        this.drivers = response.data.customers;
        ...
        validTicket() {
            let { sender_id, issuer_id, driver_id, departure, arrival } =
                this.form;
            if (sender_id == null) {
                this.$message({
                    type: "error",
            ...
            if (departure == null) {
                this.$message({
                    type: "error",
                    message: "Debe ingresar el punto de salida",
                });
                return false;
            }
            if (arrival == null) {
                this.$message({
                    type: "error",
                    message: "Debe ingresar el punto de llegada",
                });
                return false;
            }
        ...
        openDialogPersonForm(personType) {
            this.isSender = false;
            this.isIssuer = false;
            this.isDriver = false;
        ...
        async isUpdate() {
            if (this.id) {
                ...
                this.changeCurrencyType();
                await this.$http
                    .get(`/sale-notes/paymentdestinations/${this.userId}`)
                    .then((response) => {
        ...
        async saveCashDocument(package_handler_id) {
            if (!this.id) {
                await this.$http
        ...
        async submit() {
            if (!this.validTicket()) return false;
            if (!this.hasCashOpen()) {
        ...
        reloadDataCustomers(sender_id) {
            this.$http
                .get(`/${this.resource}/search/customer/${sender_id}`)
                .then((response) => {
                    this.customers = [
                    ...
                    if (this.isSender) {
                        ...
                        this.form.issuer_id = sender_id;
                        this.isIssuer = false;
                    } else {
                        this.form.driver_id = sender_id;
                        this.isDriver = false;
                    }
                    console.log(
                        "🚀 ~ file: form.vue:2431 ~ .then ~ this.customers:",
                        this.customers
                    );
                    // this.changeCustomer();
                });
        ...
        async selectDefaultCustomer() {
            if (this.config.establishment.customer_id) {
                ...
                await this.$http
                    .get(
                        `/sale-notes/search/customer/${this.config.establishment.customer_id}`
                    )
                    .then((response) => {
                ...
                if (alt !== undefined) {
                    this.form.sender_id = this.config.establishment.customer_id;
                    let seller = this.sellers.find(
