        ...
        <div class="col-md-12 col-lg-12 col-xl-12 ">
            <div class="row mt-2">
                ...
                <div class="col-md-6" style="margin-top:29px">
                    ...
                    <el-button
                        @click="openStock"
                        v-if="form.item_id && form.warehouse_id != 'all'"
                    >
                        Ver Stock inicial por meses
                    </el-button>
                </div>
            </div>
...
import StockDialog from "./ModalStockInicial.vue";
export default {
    components: { AdvancedItemsSearch, StockDialog },
    props: {
        resource: String
    },
    data() {
    ...
    computed: {
        ...
        isEnabledAdvancedRecordsSearch: function() {
            return !_.isEmpty(this.config)
                ? this.config.enabled_advanced_records_search
                : false;
        }
    },
    async mounted() {
    ...
    methods: {
        ...
        setWarehouseId() {
            if (this.warehouses.length > 0) {
                ...
                this.form.warehouse_id = !_.isEmpty(warehouse)
                    ? all_filter_id
                    : _.head(this.warehouses).id;
            }
        },
        async getCurrentWarehouse() {
            await this.$http
                .get(`/general-get-current-warehouse`)
                .then(response => {
        ...
        async changeWarehouse() {
            ...
            await this.$http
                .get(
                    `/${this.resource}/filter_by_warehouse/${
