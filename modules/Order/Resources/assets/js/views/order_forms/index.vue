                ...
                <data-table :resource="resource">
                    ...
                    <tr slot-scope="{ index, row }" :class="{'text-danger': (row.state_type_id === '11')}">
                        ...
                        <td class="text-center">
                            <button type="button" class="btn waves-effect waves-light btn-sm btn-info" @click.prevent="clickDownload(row.external_id, 'a4')">PDF</button>
                        </td>
                        <td class="text-center">
                            <a v-if="row.btn_dispatch" :href="`/order-forms/dispatch-create/${row.id}`" class="btn waves-effect waves-light btn-sm btn-primary m-1__2">Generar Guía</a>
                            <a v-if="row.btn_dispatch" :href="`/order-forms/create/${row.id}`" class="btn waves-effect waves-light btn-sm btn-warning m-1__2">Editar</a>
                            <button type="button" class="btn waves-effect waves-light btn-sm btn-info" @click.prevent="clickOptions(row.id)">Opciones</button>
                        </td>
                    </tr>
                </data-table>
            </div>
